  ...
  switch (op) {
    ...
    case BplusTreeOperationType::DELETE: {
      if (is_root_node) {  // 参考adjust_root
        ...
        // not leaf
        return size() >
               2;  // 根节点还有子节点，但是如果删除一个子节点后，只剩一个子节点，就要把自己删除，把唯一的子节点变更为根节点
      }
...
RC BplusTreeHandler::create(const char *file_name, std::vector<AttrType> attr_types, std::vector<int> attr_lengths,
  ...
  int32_t attr_total_length = std::accumulate(attr_lengths.begin(), attr_lengths.end(), 0);
  if (internal_max_size < 0) {
    internal_max_size = calc_internal_page_capacity(attr_total_length);
  }
  if (leaf_max_size < 0) {
    leaf_max_size = calc_leaf_page_capacity(attr_total_length);
  }
...
RC BplusTreeHandler::crabing_protocal_fetch_page(LatchMemo &latch_memo, BplusTreeOperationType op, PageNum page_num,
                                                 bool is_root_node, Frame *&frame) {
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid) {
  ...
  memcpy(static_cast<char *>(key.get()) + file_header_.attr_total_length, &rid, sizeof(rid));
  return key;
}
...
RC BplusTreeHandler::coalesce_or_redistribute(LatchMemo &latch_memo, Frame *frame) {
  ...
  rc = latch_memo.get_page(neighbor_page_num,
                           neighbor_frame);  // 当前已经拥有了父节点的写锁，所以直接尝试获取此页面然后加锁
  if (rc != RC::SUCCESS) {
...
template <typename IndexNodeHandlerType>
RC BplusTreeHandler::coalesce(LatchMemo &latch_memo, Frame *neighbor_frame, Frame *frame, Frame *parent_frame,
                              int index) {
  InternalIndexNodeHandler parent_node(file_header_, parent_frame);
...
RC BplusTreeScanner::open(const char *left_user_key, int left_len, bool left_inclusive, const char *right_user_key,
  ...
  if (left_user_key && right_user_key) {
    const int result = tree_handler_.key_comparator_.compare_not_with_rid(left_user_key, right_user_key);
  ...
  if (nullptr == left_user_key) {
    ...
    if (left_inclusive) {
      ...
      left_pkey = tree_handler_.make_key(right_user_key, *RID::max());
    }
  ...
  if (nullptr == right_user_key) {
    ...
    if (right_inclusive) {
      right_key_ = tree_handler_.make_key(right_user_key, *RID::max());
    } else {
      right_key_ = tree_handler_.make_key(right_user_key, *RID::min());
    }
  }
