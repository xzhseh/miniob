...
public:
  ...
  explicit AggLogicalOperator(
    const std::vector<Field> &all_keys,
  ...
  auto get_all_keys() -> std::vector<Field> & {
    return all_keys_;
  }

  auto get_agg_keys() -> std::vector<std::pair<const FieldMeta *, int>> & {
    return aggregate_keys_;
  }

  auto get_agg_types() -> std::vector<agg> & {
