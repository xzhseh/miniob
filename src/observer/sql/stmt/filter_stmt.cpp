...
RC get_table_and_field(Db *db, Table *default_table, std::unordered_map<std::string, Table *> *tables,
                       std::unordered_map<std::string, Table *> *parent_tables, const RelAttrSqlNode &attr,
                       Table *&table, const FieldMeta *&field, const std::vector<RelAttrSqlNode> &rel_attr) {
...
RC FilterStmt::create_filter_unit(Db *db, Table *default_table, std::unordered_map<std::string, Table *> *tables,
  ...
  if (condition.left_is_attr == 1) {
    ...
    rc = get_table_and_field(db, default_table, tables, parent_tables, condition.left_attr, table, field, rel_attr);
    if (rc != RC::SUCCESS) {
  ...
  if (condition.right_is_attr == 1) {
    Table *table = nullptr;
    const FieldMeta *field = nullptr;
    rc = get_table_and_field(db, default_table, tables, parent_tables, condition.right_attr, table, field, rel_attr);
    if (rc != RC::SUCCESS) {
      LOG_WARN("cannot find attr");
      return rc;
    }
